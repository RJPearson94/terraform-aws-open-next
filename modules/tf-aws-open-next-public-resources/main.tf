  ...
  ordered_cache_behaviors = concat(
    [
      for custom_error_response in local.custom_error_responses : {
        ...
        realtime_log_config_arn  = try(coalesce(try(var.behaviours.custom_error_responses.path_overrides[custom_error_response.path_pattern].realtime_log_config_arn, null), try(var.behaviours.custom_error_responses.realtime_log_config_arn, null)), null)
    ...
    flatten([
      for zone in local.zones : concat(
        [for index, image_optimisation_behaviour in lookup(zone.origins, "image_optimisation", null) == null ? [] : [for path in coalesce(try(var.behaviours.image_optimisation.zone_overrides[zone.name].paths, null), try(var.behaviours.image_optimisation.paths, null), ["/_next/image"]) : startswith(path, zone.path_prefix) ? { original : path, formatted : path } : { original : path, formatted : format("%s%s", zone.path_prefix, path) }] : {
          ...
          realtime_log_config_arn  = try(coalesce(try(var.behaviours.image_optimisation.path_overrides[image_optimisation_behaviour.formatted].realtime_log_config_arn, null), try(var.behaviours.image_optimisation.zone_overrides[zone.name].path_overrides[image_optimisation_behaviour.original].realtime_log_config_arn, null), try(var.behaviours.image_optimisation.zone_overrides[zone.name].realtime_log_config_arn, null), try(var.behaviours.image_optimisation.realtime_log_config_arn, null)), null)
        ...
        flatten([for name, origin in var.behaviours.additional_origins : [for index, additional_origin_behaviour in [for path in try(coalesce(try(origin.zone_overrides[zone.name].paths, null), origin.paths), []) : startswith(path, zone.path_prefix) ? path : format("%s%s", zone.path_prefix, path)] : {
          ...
          realtime_log_config_arn  = try(coalesce(try(origin.path_overrides[additional_origin_behaviour].realtime_log_config_arn, null), try(origin.realtime_log_config_arn, null)), null)
        ...
        [for index, server_behaviour in [for path in coalesce(try(var.behaviours.server.zone_overrides[zone.name].paths, null), try(var.behaviours.server.paths, null), ["/_next/data/*", "/api/*"]) : startswith(path, zone.path_prefix) ? { original : path, formatted : path } : { original : path, formatted : format("%s%s", zone.path_prefix, path) } if path != "*"] : {
          ...
          realtime_log_config_arn  = try(coalesce(try(var.behaviours.server.path_overrides[server_behaviour.formatted].realtime_log_config_arn, null), try(var.behaviours.server.zone_overrides[zone.name].path_overrides[server_behaviour.original].realtime_log_config_arn, null), try(var.behaviours.server.zone_overrides[zone.name].realtime_log_config_arn, null), try(var.behaviours.server.realtime_log_config_arn, null)), null)
