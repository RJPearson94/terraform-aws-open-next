locals {
  ...
  zone_origins = merge({
    ...
    image_optimisation = {
      ...
      path            = null
      use_auth_lambda = var.image_optimisation_function.backend_deployment_type == "REGIONAL_LAMBDA_WITH_AUTH_LAMBDA"
    }
    },
    ...
    { for name, additional_server_function in local.additional_server_functions : name => {
      ...
      path            = null
      use_auth_lambda = try(var.additional_server_functions.function_overrides[name].backend_deployment_type, var.additional_server_functions.backend_deployment_type) == "REGIONAL_LAMBDA_WITH_AUTH_LAMBDA"
...
module "additional_server_function" {
  ...
  function_url = {
    ...
    authorization_type = try(var.additional_server_functions.function_overrides[each.key].backend_deployment_type, var.additional_server_functions.backend_deployment_type) == "REGIONAL_LAMBDA_WITH_AUTH_LAMBDA" ? "AWS_IAM" : "NONE"
